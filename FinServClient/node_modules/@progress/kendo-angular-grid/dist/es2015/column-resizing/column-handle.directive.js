...
import { ChangeDetectorRef, Directive, ElementRef, Host, HostBinding, HostListener, Inject, Input, NgZone, Optional } from '@angular/core';
 ...
 * @hidden
 */
const createMoveStream = (service, draggable) => mouseDown => draggable.kendo.drag.pipe(takeUntil(draggable.kendo.release.pipe(tap(() => service.end()))), map(({ pageX }) => ({
...
let ColumnHandleDirective = class ColumnHandleDirective {
    constructor(draggable, element, service, zone, cdr, rtl) {
    ...
    ngOnInit() {
        ...
        this.subscriptions.add(this.zone.runOutsideAngular(() => this.draggable.kendo.press.pipe(tap(stopPropagation), tap(() => this.service.start(this.column)), switchMap(preventOnDblClick(this.draggable.kendo.release)), switchMap(createMoveStream(this.service, this.draggable)))
...
ColumnHandleDirective = tslib_1.__decorate([
    Directive({
