...
let NavigationService = class NavigationService {
    constructor(zone, domEvents, pagerContextService, scrollRequestService, groupsService, focusRoot, editService, cd, rtl) {
        this.zone = zone;
    ...
    init(meta) {
        ...
        this.domEvents.focus.pipe(takeWhile(isAlive), switchMap(onStable, args => args))
            .subscribe((args) => this.navigateTo(args.target));
        this.domEvents.focusOut.pipe(takeWhile(isAlive), filter(() => this.mode !== 0 /* Standby */), switchMap(onStable, args => args))
            .subscribe((args) => {
        ...
        this.domEvents.windowBlur.pipe(takeWhile(isAlive), filter(() => this.mode !== 0 /* Standby */))
            .subscribe(() => {
            this.mode = 0 /* Standby */;
            this.deactivateElements();
            this.cursor.announce();
        ...
        this.editService.changes.pipe(takeWhile(isAlive), filter(e => e.action !== 'edit' && this.mode === 2 /* Content */), switchMap(onStable, args => args))
        ...
        this.domEvents.keydown.pipe(takeWhile(isAlive), filter(args => args.keyCode === 9 /* tab */ && this.mode === 2 /* Content */), switchMapTo(this.domEvents.focusOut.pipe(take(1))))
            .subscribe(() => {
    ...
    navigateTo(el) {
        ...
        const focusInActiveRowContent = this.mode === 2 /* Content */ && this.activeRow === cell.rowIndex && el !== cell.element;
        if (focusInCell) {
            this.mode = 2 /* Content */;
    ...
    setViewport(firstItemIndex, lastItemIndex) {
        ...
        if (this.meta && this.meta.isVirtual && lastItemIndex > 0) {
            const dataRowIndex = Math.max(0, this.activeRow - this.meta.headerRows);
...
NavigationService = tslib_1.__decorate([
    Injectable(),
    tslib_1.__param(8, Optional()), tslib_1.__param(8, Inject(RTL)),
