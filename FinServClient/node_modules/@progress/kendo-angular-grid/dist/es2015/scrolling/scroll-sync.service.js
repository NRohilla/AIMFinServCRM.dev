...
let ScrollSyncService = class ScrollSyncService {
    constructor(ngZone) {
        ...
        this.changeSubscription.add(this.changes.pipe(distinctUntilChanged((x, y) => x.scrollLeft === y.scrollLeft))
    ...
    registerEmitter(el, sourceType) {
        ...
        if (sourceType === "body" || sourceType === "header") {
            this.ngZone.runOutsideAngular(() => {
                const obs = fromEvent(el, "scroll").pipe(map(({ target: { scrollLeft, scrollRight } }) => ({
                    scrollLeft,
                    scrollRight,
                    sourceType
                })));
                const subscription = obs.pipe(filter(x => !this.source || this.source === x.sourceType), tap(x => this.source = x.sourceType))
