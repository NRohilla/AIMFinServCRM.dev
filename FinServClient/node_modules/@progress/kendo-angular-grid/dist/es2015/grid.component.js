...
import { Component, ContentChild, ContentChildren, ElementRef, EventEmitter, HostBinding, Inject, Input, Optional, Output, Renderer2, QueryList, ViewChild, isDevMode, NgZone, ViewChildren, ChangeDetectorRef, ViewEncapsulation } from '@angular/core';
...
import { columnsToRender, expandColumns, expandColumnsWithSpan, isValidFieldName } from "./columns/column-common";
...
let GridComponent = class GridComponent {
    constructor(supportService, selectionService, wrapper, groupInfoService, groupsService, changeNotification, detailsService, editService, filterService, pdfService, responsiveService, renderer, excelService, ngZone, scrollSyncService, domEvents, rtl, columnResizingService, changeDetectorRef, columnReorderService, columnInfoService, navigationService) {
        ...
        this.columnsContainer = new ColumnsContainer(() => this.columnList.filterSort(column => !this.isHidden(column) && this.matchesMedia(column)));
        ...
        this.detailsServiceSubscription = handleExpandCollapseService(detailsService, this.detailExpand, this.detailCollapse, ({ dataItem, index }) => ({ dataItem, index }));
     ...
     * the content of current Grid columns.
     *
     * It makes sense to execute this method only
     * after the Grid is already populated with data.
     *
     ...
     * The reordering of columns operates only on the level that is inferred by the source column.
     *
    ...
    columnInstance(column) {
        ...
        if (typeof column === 'number') {
            instance = this.columnList.filter(item => !item.isColumnGroup && !item.hidden)[column];
        }
        ...
        this.ngZone.runOutsideAngular(() => {
            ...
            const windowBlurSubscription = this.renderer.listen('window', 'blur', (args) => {
                ...
                if (activeElement && !(matchesNodeName('input')(activeElement) && activeElement.type === 'file' &&
                    ...
                    closest(activeElement, matchesNodeName('kendo-grid')) === this.wrapper.nativeElement)) {
                    this.editService.closeCell(args);
                }
                this.domEvents.windowBlur.emit(args);
            });
            const clickSubscription = this.renderer.listen(wrapper, 'click', (args) => this.domEvents.click.emit(args));
            const keydownSubscription = this.renderer.listen(wrapper, 'keydown', (args) => this.domEvents.keydown.emit(args));
    ...
    resizeCheck() {
        if (window.innerWidth !== this.cachedWindowWidth && this.columnsContainer.refresh()) {
            this.ngZone.run(() => {
...
tslib_1.__decorate([
    HostBinding('class.k-grid-column-resizing'),
    tslib_1.__metadata("design:type", Boolean),
    tslib_1.__metadata("design:paramtypes", [])
], GridComponent.prototype, "columnResizing", null);
tslib_1.__decorate([
...
tslib_1.__decorate([
    ContentChild(NoRecordsTemplateDirective),
    tslib_1.__metadata("design:type", NoRecordsTemplateDirective)
], GridComponent.prototype, "noRecordsTemplate", void 0);
tslib_1.__decorate([
...
tslib_1.__decorate([
    ...
    tslib_1.__metadata("design:type", ToolbarTemplateDirective)
], GridComponent.prototype, "toolbarTemplate", void 0);
tslib_1.__decorate([
    ViewChild("lockedHeader"),
    tslib_1.__metadata("design:type", Object)
...
GridComponent = tslib_1.__decorate([
    ...
    tslib_1.__param(16, Optional()), tslib_1.__param(16, Inject(RTL)),
    tslib_1.__metadata("design:paramtypes", [BrowserSupportService,
        SelectionService,
        ElementRef,
        GroupInfoService,
        GroupsService,
        ...
        NgZone,
        ScrollSyncService,
        DomEventsService, Boolean, ColumnResizingService,
        ChangeDetectorRef,
        ColumnReorderService,
        ColumnInfoService,
        NavigationService])
